...
export const Loans = () => {
    ...
    useEffect(() => {
        const fetchUserCurrentLoans = async () => {
            ...
            if(authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/api/bookEntities/secure/currentloans`;
                ...
                if(!shelfCurrentLoansResponse.ok) {
                    throw new Error("Soemthing went wrong - shelfloans!");
                }
    ...
    async function returnBook(bookId: number) {
        const url = `http://localhost:8080/api/bookEntities/secure/return?bookId=${bookId}`;
        const requestOptions = {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
        ...
        if(!returnResponse.ok) {
            throw new Error("Something went wrong - return book!")
        }
        setCheckout(!checkout);
    }

    async function renewLoan(bookId: number) {
        const url = `http://localhost:8080/api/bookEntities/secure/renew/loan?bookId=${bookId}`;
        const requestOptions = {
            method: "PUT",
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
        ...
        if (!returnResponse.ok){
            throw new Error("Something went wrong - renew loan!")
        }
        setCheckout(!checkout);
    }
                            ...
                            <div className="row mt-3 mb-3">
                                ...
                                <div className="card col-3 col-md-3 container d-flex">
                                    <div className="card-body">
                                        ...
                                        <Link to={`/checkout/${shelfCurrentLoan.book.id}`} className="btn btn-primary">Leave a review</Link>
                                    </div>
                                </div>
                            </div>
                            <hr/>
